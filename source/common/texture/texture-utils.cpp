...
our::Texture2D *our::texture_utils::loadImage(const std::string &filename, bool generate_mipmap)
{
    ...
    unsigned char *pixels = stbi_load(filename.c_str(), &size.x, &size.y, &channels, 4); // FORCE 4 channels
    if (pixels == nullptr)
    {
        std::cerr << "[ERROR] Failed to load image: " << filename << " | Reason: " << stbi_failure_reason() << std::endl;
        return nullptr;
    }
    // Create a texture
    our::Texture2D *texture = new our::Texture2D();
    ...
    // Since we forced 4 channels in stbi_load, we can safely upload as GL_RGBA
